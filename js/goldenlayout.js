...
lm.utils.copy( lm.controls.BrowserPopout.prototype, {
	...
	_createUrl: function() {
		...
		urlAndContext = document.location.href.split('#');
		urlParts = urlAndContext[0].split( '?' );

		var result;
		// URL doesn't contain GET-parameters
		if( urlParts.length === 1 ) {
			result = urlParts[ 0 ] + '?gl-window=' + storageKey;

		// URL contains GET-parameters
		} else {
			result = urlParts.join("?") + '&gl-window=' + storageKey;
		}
		if (urlAndContext.length > 1) {
			result += "#" + urlAndContext[1];
		}
		return result;
	},
